...
void Trainer2D::trainModel(std::vector<float> trainLossHistory, std::vector<float> trainAccuracyHistory,
                           std::vector<float> testLossHistory, std::vector<float> testAccuracyHistory,
                           Sequential& model, uint32_t epochsCount,
                           const Eigen::MatrixXf& trainTarget,const Eigen::MatrixXf& trainFeatures,
                            const Eigen::MatrixXf& testTarget,const Eigen::MatrixXf& testFeatures,
...
{           
    ...
    for(uint32_t i =0; i< epochsCount; i++)
    {
        ...
        if(i%verboseFrequence==0)
            std::cout << "Epoch: " << i << ", train accuracy: " << trainAccuracyHistory.at(i+1) << ", loss: " << loss << ", test accuracy: " << testAccuracyHistory.at(i+1) << std::endl; 
    }}
